...
*       proc calcularTramoAscendente(vector<int> v, numElemsDer) dev {numElems, maxAncho}
*       PostCondición Q = (#numElems = for all i : 0 <= i < v.size() : (v[i] <= v[i + 1]) ^ max(#ancho : v[i] == v[i + 1]))
